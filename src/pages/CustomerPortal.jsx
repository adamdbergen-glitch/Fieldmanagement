...
import { 
  ...
  Hammer, MessageSquare, Image as ImageIcon, Send, DollarSign, Sparkles, Phone, Mail, LayoutDashboard, FileSignature, X, ListPlus, Download // NEW: Added Download icon
} from 'lucide-react'
...
// NEW: Updated to call the backend endpoint securely instead of writing directly to the database
  ...
  return (
    <div className="bg-slate-50 min-h-screen">
      {/* NEW: Wrapper to hide interactive UI when printing */}
      <div className="py-10 px-4 sm:px-6 relative overflow-hidden print:hidden">
                  ...
                  <div className="mt-auto pt-6 border-t border-white/10">
                    ...
                    {isEstimatePhase && (
                      <div className="max-w-md mx-auto w-full">
                        ...
                        {/* NEW: PDF Quote Download Button */}
                        <button 
                          onClick={() => window.print()}
                          className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 px-6 rounded-xl transition-all flex items-center justify-center gap-2 text-md mt-3"
                        >
                          <Download size={20} />
                          Download PDF Quote
                        </button>
      ...
      {/* --- NEW: PRINT ONLY VIEW (COMMERCIAL PDF QUOTE) --- */}
      <div className="hidden print:block bg-white text-black font-sans max-w-4xl mx-auto p-8">
        {/* Quote Header */}
        <div className="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-6">
          <div>
            <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tight">{APP_CONFIG.appName}</h1>
            <p className="text-slate-600">Winnipeg, MB | {APP_CONFIG.companyPhone}</p>
          </div>
          <div className="text-right">
            <h2 className="text-2xl font-bold text-slate-400 uppercase tracking-widest">Quote / Estimate</h2>
            <p className="text-slate-600 mt-1 font-medium">Date: {format(new Date(), 'MMMM do, yyyy')}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          <div>
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">Prepared For:</h3>
            <p className="text-lg font-bold text-slate-900">{displayName}</p>
            {project.customer_email && <p className="text-slate-600">{project.customer_email}</p>}
            {(project.address || project.city) && <p className="text-slate-600">{project.address} {project.city}</p>}
          </div>
          <div>
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">Project Details:</h3>
        ...
        {project.scope_of_work && (
          <div className="mb-8">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-2 border-b border-slate-200 pb-1">Scope of Work</h3>
            <p className="whitespace-pre-wrap text-slate-700 text-sm">{project.scope_of_work}</p>
          </div>
        )}

        {/* Line Items Table */}
        <div className="mb-8">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-2 border-b border-slate-200 pb-1">Investment Breakdown</h3>
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b-2 border-slate-900 text-slate-900">
                <th className="py-2 font-bold">Description</th>
                <th className="py-2 text-right font-bold w-32">Amount</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {lineItems?.filter(i => checkedItems[i.id]).map(item => (
                <tr key={item.id}>
                  <td className="py-3">
                    <p className="font-bold text-slate-900">{item.title}</p>
                    <p className="text-xs text-slate-600">{item.description}</p>
                  </td>
                  <td className="py-3 text-right font-mono text-slate-900">${Number(item.price).toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex justify-end mb-12">
          <div className="w-64 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-mono text-slate-900">${dynamicSubtotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
            </div>
            <div className="flex justify-between text-sm border-b border-slate-200 pb-2">
              <span className="font-bold text-slate-600">GST (5%):</span>
              <span className="font-mono text-slate-900">${dynamicGST.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
            </div>
            <div className="flex justify-between text-lg pt-2">
              <span className="font-black text-slate-900">Grand Total:</span>
              <span className="font-black font-mono text-slate-900">${dynamicTotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
            </div>
          </div>
        </div>

        {/* Terms and Conditions */}
        <div className="text-xs text-slate-500 whitespace-pre-wrap border-t border-slate-200 pt-6 break-inside-avoid">
          <p className="font-bold text-slate-700 mb-2 text-sm uppercase tracking-widest">Terms & Conditions:</p>
          {CONTRACT_TERMS}
        </div>
        ...
        <div className="mt-12 pt-8 border-t-2 border-slate-900 grid grid-cols-2 gap-8 break-inside-avoid">
          <div>
            <p className="text-sm font-bold text-slate-900 mb-8">Authorized Signature (Client):</p>
            <div className="border-b border-slate-400"></div>
            <p className="text-xs text-slate-500 mt-2">Date: __________________</p>
          </div>
          <div>
            <p className="text-sm font-bold text-slate-900 mb-8">Authorized Signature (Contractor):</p>
            <div className="border-b border-slate-400"></div>
            <p className="text-xs text-slate-500 mt-2">Date: __________________</p>
          </div>
        </div>

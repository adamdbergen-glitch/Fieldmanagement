...
export default function CustomerPortal() {
  ...
  return (
    <>
      {/* ========================================= */}
      {/* PRINT-ONLY INVOICE TEMPLATE (HIDDEN IN UI) */}
      {/* ========================================= */}
      <div className="hidden print:block bg-white text-black p-8 font-sans w-full mx-auto">
        {/* Header */}
        <div className="flex justify-between items-start border-b-2 border-slate-800 pb-6 mb-6">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-tight">The Paving Stone Pros</h1>
            <p className="text-slate-600 mt-1">Winnipeg, MB | (204) 803-6464</p>
          </div>
          <div className="text-right">
            <h2 className="text-2xl font-bold text-slate-400 uppercase tracking-widest">Quote / Estimate</h2>
            <p className="font-bold mt-2">Date: {format(new Date(), 'MMMM do, yyyy')}</p>
          </div>
        </div>
        ...
        <div className="flex justify-between mb-8">
          <div>
            ...
            <p className="font-bold text-lg">{signatureName || displayName}</p>
            {(signaturePhone || project.customer_phone) && <p>{signaturePhone || project.customer_phone}</p>}
            {(signatureEmail || project.customer_email) && <p>{signatureEmail || project.customer_email}</p>}
          </div>
          <div className="text-right">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-1">Project Details:</h3>
            <p className="font-bold">{project.name}</p>
            <p>{signatureAddress || project.address || project.customer_address}</p>
          </div>
        </div>
        ...
        {project.scope_of_work && (
          <div className="mb-8">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200 pb-2 mb-3">Scope of Work</h3>
        ...
        <div className="mb-8 break-inside-avoid">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200 pb-2 mb-3">Investment Breakdown</h3>
          <table className="w-full text-sm text-left border-collapse">
            ...
            <tbody>
              {lineItems?.filter(item => checkedItems[item.id]).map(item => (
                <tr key={item.id} className="border-b border-slate-200">
                  <td className="p-3">
                    <p className="font-bold">{item.title}</p>
                    <p className="text-slate-500 text-xs mt-1">{item.description}</p>
                  </td>
                  <td className="p-3 text-right font-bold whitespace-nowrap">${Number(item.price).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex justify-end mt-4">
            <div className="w-64 space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span>${dynamicSubtotal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
              </div>
              <div className="flex justify-between">
                <span className="font-bold text-slate-500">GST (5%):</span>
                <span>${dynamicGST.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
              </div>
              <div className="flex justify-between border-t border-slate-800 pt-2 text-lg">
                <span className="font-black">Grand Total:</span>
                <span className="font-black">${dynamicTotal.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Contract / Terms */}
        <div className="mb-8">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest border-b border-slate-200 pb-2 mb-3">Terms & Conditions</h3>
          <div className="text-[10px] leading-relaxed whitespace-pre-wrap text-slate-600 columns-2 gap-8">
            {CONTRACT_TERMS}
          </div>
        </div>

        {/* Signatures */}
        <div className="flex justify-between mt-16 break-inside-avoid pb-8">
          <div className="w-5/12 border-t border-black pt-2">
            <p className="text-xs font-bold">Authorized Signature (Client)</p>
            <p className="text-xs mt-1">Date: ________________________</p>
          </div>
          <div className="w-5/12 border-t border-black pt-2">
            <p className="text-xs font-bold">Authorized Signature (Contractor)</p>
            <p className="text-xs mt-1">Date: ________________________</p>
          </div>
        </div>
      </div>
      ...
      {/* --- STANDARD DIGITAL UI (HIDDEN DURING PRINT) --- */}
      <div className="min-h-screen py-10 px-4 sm:px-6 relative overflow-hidden print:hidden">
